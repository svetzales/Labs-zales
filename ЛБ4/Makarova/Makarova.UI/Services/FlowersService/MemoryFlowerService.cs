...
namespace Yeroma.UI.Services.FlowersService
{
    public class MemoryFlowersService : IFlowersService
    {
        ...
        public MemoryFlowersService(ICategoryService categoryService)
        {
        ...
        public Task<ResponseData<ListModel<Flower>>> GetFlowersListAsync(string? categoryNormalizedName, int pageNo = 1)
        {
            // Создать объект результата
            var result = new ResponseData<ListModel<Flower>>();
            ...
            result.Data = new ListModel<Flower>() { Items = data };
            //Если список пустой
            if (data.Count == 0)
            {
