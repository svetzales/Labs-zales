...
using Yeroma.API.Controllers;
using Yeroma.API.Data;

namespace Yeroma.Tests
{
    public class FlowersControllerTests : IDisposable
    {
        ...
        public FlowersControllerTests()
        {
            ...
            {
                new Category {Name="", NormalizedName="packaging"},
                new Category {Name="", NormalizedName="box"},
                new Category {Name="", NormalizedName="basket"}
            };
            context.Categories.AddRange(categories);
            context.SaveChanges();
            var flowers = new List<Flower>
            {
                new Flower {Name="", Description="", Price=0, Category=categories.FirstOrDefault(c=>c.NormalizedName.Equals("packaging"))},
                new Flower {Name = "", Description="", Price=0, Category=categories.FirstOrDefault(c=>c.NormalizedName.Equals("box"))},
                new Flower {Name = "", Description="", Price=0,Category=categories.FirstOrDefault(c=>c.NormalizedName.Equals("basket"))},

            };
            context.AddRange(flowers);
            context.SaveChanges();
        }
        ...
        public async void ControllerFiltersCategory()
        {
            ...
            var controller = new Flowers1Controller(context, _environment);
            // act
            var response = await controller.GetFlower(category.NormalizedName);
            ResponseData<ProductListModel<Flower>> responseData = response.Value;
        ...
        public async void ControllerReturnsCorrectPagesCount(int size, int qty)
        {
            ...
            var controller = new Flowers1Controller(context, _environment);
            // act
            var response = await controller.GetFlower(null, 1, size);
            ResponseData<ProductListModel<Flower>> responseData = response.Value;
        ...
        public async void ControllerReturnsCorrectPage()
        {
            ...
            Flower firstItem = context.Flowers.ToArray()[3];
            // act
            // Получить данные 2-й страницы
            var response = await controller.GetFlower(null, 2);
            ResponseData<ProductListModel<Flower>> responseData = response.Value;
            var flowersList = responseData.Data.Items; // полученный список объектов
            ...
            Assert.Equal(2, flowersList.Count); // количество объектов на странице равно 2
            Assert.Equal(firstItem.Id, flowersList[0].Id); // 1-й объект в списке правильный
        }
    }
}
